...
- **User Pool ID**: ID del pool de usuarios de Cognito
- **App Client ID**: ID del cliente de la aplicación

### 2. Probar la Aplicación

1. Abre la Frontend URL en tu navegador
2. Crea una cuenta nueva en la página de registro
3. Verifica tu email con el código de confirmación
4. Inicia sesión con tus credenciales
5. Sube un PDF de prueba
6. Verifica que el análisis se complete correctamente

## Autenticación con Amazon Cognito

### Configuración Automática

La autenticación con Cognito se configura automáticamente durante el deployment:

- User Pool con verificación por email
- App Client sin secreto (para uso en frontend)
- Lambda Authorizer para proteger las APIs
...
3. **Login**: El usuario inicia sesión y recibe tokens JWT
4. **Acceso**: Los tokens se incluyen automáticamente en las llamadas API
5. **Refresh**: Los tokens se renuevan automáticamente antes de expirar

### Recuperación de Contraseña

1. El usuario solicita recuperación desde la página de login
2. Se envía un código de verificación al email
3. El usuario ingresa el código y la nueva contraseña
4. La contraseña se actualiza y puede iniciar sesión

### Troubleshooting de Autenticación

#### Error: "User not confirmed"

**Solución**: El usuario debe verificar su email con el código de confirmación
...
**Solución**: El sistema debería renovar automáticamente. Si persiste, cierra sesión y vuelve a entrar

#### Error: "Access denied" en API

**Solución**: Verifica que el token se está enviando correctamente en el header Authorization
...
- Lambda Authorizer para validación de tokens
- Verificación de email obligatoria

### Para Producción

- Configurar WAF para protección adicional
- Implementar logging de auditoría avanzado
