// Dashboard functionality - Updated 2025-12-12 18:17 with delete functionality
...
async function loadDashboardKPIs() {
    try {
        // Load recent analyses to calculate KPIs
        ...
        const completedAnalyses = recentAnalyses.filter(analysis => analysis.status === 'COMPLETED');
        ...
        if (completedAnalyses.length > 0) {
            const avgScore = completedAnalyses.reduce((sum, analysis) => {
        ...
        const highRiskCount = completedAnalyses.filter(analysis => 
            (analysis.aiLikelihoodScore || 0) > threshold
        ).length;
        updateKPICard('highRiskCount', highRiskCount);
        
        // Calculate average originality score
        if (completedAnalyses.length > 0) {
            const avgOriginality = completedAnalyses.reduce((sum, analysis) => {
        ...
        if (completedAnalyses.length > 0) {
            const avgConfidence = completedAnalyses.reduce((sum, analysis) => {
        ...
        loadRecentAnalysesTable(analyses.slice(0, 5));
        ...
        return `
            <tr>
                ...
                <td>
                    ...
                    ${analysis.metadata?.subject ? `<small class="text-muted">${analysis.metadata.subject}</small>` : ''}
                </td>
                <td>
                    <div>${formatDate(analysis.createdAt)}</div>
                    <small class="text-muted">${getTimeAgo(analysis.createdAt)}</small>
                </td>
                <td>
        ...
        tableBody.innerHTML = analyses.map(analysis => {
            ...
            return `
                <tr>
                    ...
                    <td>
                        <div class="fw-medium">${analysis.assignmentName || '-'}</div>
                    </td>
                    <td>
                        <div>${formatDate(analysis.createdAt)}</div>
                        <small class="text-muted">${getTimeAgo(analysis.createdAt)}</small>
                    ...
                    <td>
                        <span class="badge badge-${getScoreColor(aiScore)} fs-6">
                            ${aiScore}%
                        </span>
                    </td>
                    <td>
                        <span class="badge badge-${getScoreColor(100 - originalityScore)} fs-6">
                            ${originalityScore}%
                        </span>
                    </td>
                    <td>${getStatusBadge(analysis.status)}</td>
...
async function downloadPDF(analysisId) {
    try {
        ...
        const response = await apiCall(`/downloads/presign?analysisId=${analysisId}`);
