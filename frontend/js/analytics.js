...
function showAnalyticsSection() {
  ...
  try {
    ...
    if (typeof hideAllSections === "function") {
      ...
      console.warn("hideAllSections function not found");
      // Fallback: hide common sections manually
    ...
    if (pageTitle) {
      pageTitle.textContent = "Analytics y Reportes";
    }
    ...
    if (typeof updateActiveNavItem === "function") {
      ...
      console.warn("updateActiveNavItem function not found");
      // Fallback: update nav items manually
      document.querySelectorAll(".nav-item").forEach((item) => {
        item.classList.remove("active");
        if (item.textContent.includes("Analytics")) {
    ...
    console.error("Error showing analytics section:", error);
    showError("Error al mostrar la sección de Analytics: " + error.message);
  }
}
...
function updateTopCourses(analyses) {
  ...
  tableBody.innerHTML = topCourses
    .map(
      (item, index) => `
        <tr>
...
function updateTopStudents(analyses) {
  ...
  tableBody.innerHTML = topStudents
    .map(
      (analysis, index) => `
        <tr>
...
async function exportDashboardToPDF() {
  try {
    ...
    const avgAiScore =
      document.getElementById("avgAiScore")?.textContent || "0%";
    const highRiskCount =
      document.getElementById("highRiskCount")?.textContent || "0";
    const avgOriginalityScore =
      document.getElementById("avgOriginalityScore")?.textContent || "0%";
    const todayAnalyses =
      document.getElementById("todayAnalyses")?.textContent || "0";
    const avgConfidence =
...
async function exportDashboardToExcel() {
  try {
    ...
    // Create CSV content
    let csvContent =
      "Estudiante,Curso,Tarea,Fecha,IA Score,Originalidad,Confianza,Estado\n";
...
async function exportAnalyticsToExcel() {
  try {
    ...
    let csvContent =
      "Estudiante,Curso,Tarea,Fecha,IA Score,Originalidad,Confianza,Estado,Señales Detectadas\n";
